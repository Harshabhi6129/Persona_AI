...
def detect_personality_mode(query):
    ...
    response = openai.chat.completions.create(
        model=\"gpt-4\",
        messages=[
            {\"role\": \"system\", \"content\": \"You are a classifier that determines the appropriate conversational tone.\"},
...
def detect_sentiment(query):
    ...
    response = openai.chat.completions.create(
        model=\"gpt-4\",
        messages=[
            {\"role\": \"system\", \"content\": \"You are an AI sentiment analyzer.\"},
...
def generate_response(query):
    """
    Retrieve relevant context and generate a response with sentiment adaptation.
    """
    # These are placeholders now: returns (None, persona_docs, memory_docs, correction_docs)
    client, persona_collection, memory_collection, correction_collection = build_chroma_collection()
    ...
    response = openai.chat.completions.create(
        model=\"gpt-4\",
        messages=[
            {\"role\": \"system\", \"content\": \"You are John Doe, a highly personalized AI.\"},
            {\"role\": \"user\", \"content\": prompt}
        ]
    )
    final_response = response.choices[0].message.content

    # Store the memory so it can be retrieved later
